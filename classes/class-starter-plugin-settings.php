...
final class Starter_Plugin_Settings
{
    ...
    public function validate_settings($input, $section)
    {
        if (is_array($input) && 0 < count($input)) {
            ...
            foreach ($input as $k => $v) {
                ...
                if ('' != $method) {
                    add_filter('starter-plugin-validate-field-'.$fields[$k]['type'], [$this, $method]);
                }
 ...
 // End validate_field_url()
    ...
    public function render_field($args)
    {
        ...
        if (isset($args['type']) && !in_array($args['type'], (array) apply_filters('starter-plugin-no-description-fields', ['checkbox']))) {
            if (isset($args['description'])) {
                $description = '<p class="description">'.wp_kses_post($args['description']).'</p>'."\n";
                if (in_array($args['type'], (array) apply_filters('starter-plugin-new-line-description-fields', ['textarea', 'select']))) {
    ...
    public function get_settings_fields($section)
    {
        ...
        switch ($section) {
            ...
            case 'special-fields':
                ...
                $settings_fields['select_taxonomy'] = [
 ...
 // End get_no_label_field_types()
    ...
    {
        return (array) apply_filters('starter-plugin-supported-fields', ['text', 'checkbox', 'radio', 'textarea', 'select', 'select_taxonomy']);
    }
 ...
 // End get_supported_fields()
    ...
    {
        $values = get_option('starter-plugin-'.$section, []);
    ...
    public function get_settings($section = '')
    {
        ...
        if (0 < count($sections)) {
            foreach ($sections as $k => $v) {
                ...
                $values = get_option('starter-plugin-'.$v, []);
